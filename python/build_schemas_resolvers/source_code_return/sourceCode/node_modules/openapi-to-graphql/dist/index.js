...
const GraphQLUpload = _interopDefault(require('graphql-upload/GraphQLUpload.js'));
...
function sanitize(str, caseStyle) {
     ...
     */
    if (caseStyle === exports.CaseStyle.simple) {
     ...
     */
    const regex = caseStyle === exports.CaseStyle.ALL_CAPS
        ? /[^a-zA-Z0-9_]/g // ALL_CAPS has underscores
        : /[^a-zA-Z0-9]/g;
    let sanitized = str.split(regex).reduce((path, part) => {
        if (caseStyle === exports.CaseStyle.ALL_CAPS) {
            return path + '_' + part;
        }
        else {
            return path + capitalize(part);
...
function generateOperationId(method, path) {
    return sanitize(`${method} ${path}`, exports.CaseStyle.camelCase);
}
...
const pubsubLog = debug.debug('pubsub');
const uploadLog = debug.debug('fileUpload');
...
function getSubscribe({ operation, payloadName, data, baseUrl, connectOptions }) {
    ...
    return (root, args, context, info) => {
         ...
         * lookup here
         */
        const paramName = sanitize(payloadName, exports.CaseStyle.camelCase);
        let resolveData = {};
        if (payloadName && typeof payloadName === 'string') {
            ...
            const sanePayloadName = data.options.genericPayloadArgName
                ? 'requestBody'
                : sanitize(payloadName, exports.CaseStyle.camelCase);
    ...
    return async (source, args, context, info) => {
        ...
        operation.parameters.forEach((param) => {
            const saneParamName = sanitize(param.name, !data.options.simpleNames
                ? exports.CaseStyle.camelCase
                : exports.CaseStyle.simple);
...
function extractToken(data, context) {
    ...
    const tokens = jsonpathPlus.JSONPath({
        path: tokenJSONpath,
        json: context
    });
...
function getAuthReqAndProtcolName(operation, _openAPIToGraphQL) {
    ...
    if (Array.isArray(operation.securityRequirements) &&
        ...
        for (let securityRequirement of operation.securityRequirements) {
            const sanitizedSecurityRequirement = sanitize(securityRequirement, exports.CaseStyle.camelCase);
...
function resolveRuntimeExpression(paramName, runtimeExpression, resolveData, root, args) {
    if (runtimeExpression === '$url') {
        ...
        if (runtimeExpression === '$request.body') {
            ...
            return resolveData.usedParams[sanitize(runtimeExpression.split('query.')[1], exports.CaseStyle.camelCase)];
            // CASE: parameter in previous path parameter
        }
        else if (runtimeExpression.startsWith('$request.path')) {
            return resolveData.usedParams[sanitize(runtimeExpression.split('path.')[1], exports.CaseStyle.camelCase)];
        ...
        for (let path in oas.paths) {
            ...
            Object.keys(pathItem)
                ...
                let operationType = httpMethod === HTTP_METHODS.get
                    ? exports.GraphQLOperationType.Query
                    : exports.GraphQLOperationType.Mutation;
                ...
                // This is an enum, which is an integer value
                    ...
                    operationType =
                        options.selectQueryOrMutationField[oas.info.title][path][httpMethod] === exports.GraphQLOperationType.Mutation
                            ? exports.GraphQLOperationType.Mutation
                            : exports.GraphQLOperationType.Query;
                }
                    ...
                    Object.entries(operation.callbacks).forEach(([callbackName, callbackObjectOrRef]) => {
                        ...
                        Object.entries(callback).forEach(([callbackExpression, callbackPathItem]) => {
                            ...
                            if (callbackOperationObjectMethods.length > 0) {
                                ...
                                const callbackOperation = processOperation(callbackExpression, callbackHttpMethod, callbackOperationString, exports.GraphQLOperationType.Subscription, resolvedCallbackPathItem[callbackHttpMethod], callbackPathItem, oas, data, options);
    ...
    for (let schemeKey in security) {
        ...
        switch (securityScheme.type) {
            case 'apiKey':
                ...
                parameters = {
                    apiKey: sanitize(`${schemeKey}_apiKey`, exports.CaseStyle.camelCase)
                };
                schema = {
                    type: 'object',
                    description,
            ...
            case 'http':
                switch (securityScheme.scheme) {
                    ...
                    case 'basic':
                        ...
                        parameters = {
                            username: sanitize(`${schemeKey}_username`, exports.CaseStyle.camelCase),
                            password: sanitize(`${schemeKey}_password`, exports.CaseStyle.camelCase)
                        };
                        schema = {
                            type: 'object',
                            description,
                    ...
                    case 'bearer':
                        ...
                        parameters = {
                            token: sanitize(`${schemeKey}_token`, exports.CaseStyle.camelCase)
                        };
                        schema = {
                            type: 'object',
                            description,
...
function createDataDef(names, schemaOrRef, isInputObjectType, data, oas, links) {
    ...
    if (name === names.fromExtension) {
        ...
        saneName = !data.options.simpleNames
            ? sanitize(name, exports.CaseStyle.PascalCase)
            : capitalize(sanitize(name, exports.CaseStyle.simple));
...
function getPreferredName(names) {
    if (typeof names.preferred === 'string') {
        return sanitize(names.preferred, exports.CaseStyle.PascalCase); // CASE: preferred name already known
    }
    else if (typeof names.fromRef === 'string') {
        return sanitize(names.fromRef, exports.CaseStyle.PascalCase); // CASE: name from reference
    }
    else if (typeof names.fromSchema === 'string') {
        return sanitize(names.fromSchema, exports.CaseStyle.PascalCase); // CASE: name from schema (i.e., "title" property in schema)
    }
    else if (typeof names.fromPath === 'string') {
        return sanitize(names.fromPath, exports.CaseStyle.PascalCase); // CASE: name from path
    }
    else {
...
function getSchemaName(names, usedNames) {
    ...
    if (!schemaName && typeof names.fromRef === 'string') {
        const saneName = sanitize(names.fromRef, exports.CaseStyle.PascalCase);
    ...
    if (!schemaName && typeof names.fromSchema === 'string') {
        const saneName = sanitize(names.fromSchema, exports.CaseStyle.PascalCase);
        if (!usedNames.includes(saneName)) {
    ...
    if (!schemaName && typeof names.fromPath === 'string') {
        const saneName = sanitize(names.fromPath, exports.CaseStyle.PascalCase);
        if (!usedNames.includes(saneName)) {
            ...
            : typeof names.fromRef === 'string'
                ...
                : typeof names.fromSchema === 'string'
                    ...
                    : typeof names.fromPath === 'string'
                        ? names.fromPath
                        : 'PlaceholderName', exports.CaseStyle.PascalCase);
    }
 ...
 */
const CleanGraphQLJSON = new graphql.GraphQLScalarType({
    ...
    serialize: (value) => {
        ...
        return graphqlScalars.GraphQLJSON.serialize(cleanValue);
    }
});
...
function getGraphQLType({ def, operation, data, iteration = 0, isInputObjectType = false, fetch }) {
    ...
    switch (def.targetGraphQLType) {
        ...
        case TargetGraphQLType.bigint:
            def.graphQLType = graphqlScalars.GraphQLBigInt;
            return def.graphQLType;
...
function createAndLoadViewer(queryFields, operationType, data, fetch) {
    ...
    for (let protocolName in queryFields) {
        ...
        let viewerName = operationType === exports.GraphQLOperationType.Query
            ? sanitize(`viewer ${viewerType}`, exports.CaseStyle.camelCase)
            : operationType === exports.GraphQLOperationType.Mutation
                ? sanitize(`mutation viewer ${viewerType}`, exports.CaseStyle.camelCase)
                : sanitize(`subscription viewer ${viewerType}`, exports.CaseStyle.camelCase);
    ...
    const anyAuthObjectName = operationType === exports.GraphQLOperationType.Query
        ? 'viewerAnyAuth'
        : operationType === exports.GraphQLOperationType.Mutation
...
function getViewerOT(name, protocolName, securityType, queryFields, data) {
    ...
    const resolve = (source, args, context, info) => {
        const security = {};
        const saneProtocolName = sanitize(protocolName, exports.CaseStyle.camelCase);
    ...
    if (typeof scheme === 'object') {
        for (let parameterName in scheme.parameters) {
            ...
            const saneParameterName = sanitize(parameterName, exports.CaseStyle.camelCase);
            args[saneParameterName] = { type: new graphql.GraphQLNonNull(graphql.GraphQLString) };
        }
    }
...
provideErrorExtensions, equivalentToMessages, fetch }) {
    ...
    Object.entries(data.operations).forEach(([operationId, operation]) => {
        ...
        if (operation.operationType === exports.GraphQLOperationType.Query) {
            addQueryFields({
                authQueryFields,
                queryFields,
                operationId,
        ...
        else if (operation.operationType === exports.GraphQLOperationType.Mutation) {
            addMutationFields({
                authMutationFields,
                mutationFields,
                operationId,
    ...
    if (Object.keys(authQueryFields).length > 0) {
        Object.assign(queryFields, createAndLoadViewer(authQueryFields, exports.GraphQLOperationType.Query, data, fetch));
    }
    if (Object.keys(authMutationFields).length > 0) {
        Object.assign(mutationFields, createAndLoadViewer(authMutationFields, exports.GraphQLOperationType.Mutation, data, fetch));
    }
    if (Object.keys(authSubscriptionFields).length > 0) {
        Object.assign(subscriptionFields, createAndLoadViewer(authSubscriptionFields, exports.GraphQLOperationType.Subscription, data, fetch));
    }
    // Build up the schema
    const schemaConfig = {
...
function addQueryFields({ authQueryFields, queryFields, operationId, operation, options, data }) {
    ...
    const saneOperationId = sanitize(operationId, exports.CaseStyle.camelCase);
    ...
    const generatedFieldName = operationIdFieldNames
        ...
        : singularNames
            ...
            inferResourceNameFromPath(operation.path), exports.CaseStyle.camelCase)
            : uncapitalize(
...
function addMutationFields({ authMutationFields, mutationFields, operationId, operation, options, data }) {
    ...
    const generatedFieldName = singularNames
        ...
        `${operation.method}${inferResourceNameFromPath(operation.path)}`, exports.CaseStyle.camelCase)
...
function getFieldForOperation(operation, baseUrl, data, requestOptions, fileUploadOptions, connectOptions, fetch) {
    ...
    if (operation.operationType === exports.GraphQLOperationType.Subscription) {
...
exports.translateOpenAPIToGraphQL = translateOpenAPIToGraphQL;
